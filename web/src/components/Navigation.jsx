      ...
      <div className="h-full flex flex-col glass">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar">
          <div className="p-4 space-y-6">
            {Object.entries(files.reduce((acc, note) => {
              const day = note.day;
              if (!acc[day]) acc[day] = [];
              acc[day].push(note);
              return acc;
              ...
              <div key={day} className="space-y-1">
                ...
                {dayNotes.map((note) => {
                  ...
                  // Simplify title if it contains "Part X" and we are in a group
                  let displayTitle = title;
                  if (dayNotes.length > 1) {
                    // Optional: could strip "Day X" or "Part X" here if redundant
                  }
                  ...
                  return (
                    <button
                      ...
                      className={`
                        w-full text-left p-3 pl-8 flex items-center gap-4 transition-all duration-300 relative group overflow-hidden
                        ${isActive 
